...
public class MultiThreadedClient {
   ...
   * Attempts to create an user account with given credentials
   * 
   * @param text the group to join
   * @return whether or not the join was successful
   */
  public void attemptGroupJoin(String text) {
    sendGroupJoin(text);
  }
  ...
  void sendGroupJoin(String group_id){
    CmdPacket cmd = new CmdPacket("GROUPJOIN",username,"", group_id, c_nonce,s_nonce);
    String data = CryptoAlg.encodeObject(cmd);
    AESPacket aesp = encAES(data);
